...
print("final memory drop 0.05 both dir")
# paths
...
class RMCArguments:
    def __init__(self):
        self.memslots = 1
        self.numheads = 8
        self.headsize = int((1024 * self.memslots) / (self.numheads * self.memslots)) # 128 need to make 1024
...
test['s1'] = test['s1'][0:-9]
test['s2'] = test['s2'][0:-9]
test['label'] = test['label'][0:-9]

valid['s1'] = valid['s1'][0:-9]
valid['s2'] = valid['s2'][0:-9]
valid['label'] = valid['label'][0:-9]
