...
def map_label_to_target(label, num_classes):
    ...
    target = np.zeros((1, num_classes), dtype=float)
...
def get_SICK_labels():
    with open("dataset/SICK_labels.txt", "r") as f:
        data = f.readlines()
    train, valid, test, valid_org, test_org = [], [], [], [], []
    for i in range(4500):
        x = float(data[i].rstrip())
        x = map_label_to_target(x, 5)
        train.append(x)
    ...
    for i in range(5000,len(data)):
        ...
        test.append(x)
    test = np.array(test)
    return train, valid, test, valid_org, test_org
