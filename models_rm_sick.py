...
class InferSent(nn.Module):
    ...
    def __init__(self, config, RM):
        ...
        #self.proj_enc = nn.Linear(1024, 1024, bias=False)

        self.relational_memory = RM
    ...
    def forward(self, sent_tuple, memory):
        # sent_len: [max_len, ..., min_len] (bsize)
        # sent: (seqlen x bsize x worddim)
        sent, sent_len = sent_tuple
        #print(sent_len, sent.size(), memory.size())
        
        sent_output = self.relational_memory(sent.transpose(0, 1), memory) # torch.Size([25, 1, 4800])
        
        #sent_output = self.proj_enc(sent_output)
        '''
        sent_output = sent_output.transpose(0,1)
        emb = torch.zeros(sent_output.shape[0], sent_output.shape[2]).cuda()
        for i in range(len(sent_len)):
            emb[i] = sent_output[i, sent_len[i]-1]
        return emb
        '''
        '''
        for i in range(len(sent_len)):
            for j in range(sent_len[i], sent_output.shape[1]):
                sent_output[i, j] = torch.zeros(sent_output.shape[2])
        sent_output = sent_output.transpose(0,1)
        '''
        
        return sent_output.squeeze(1)
...
class NLINet(nn.Module):
    def __init__(self, config, RM):
        ...
        self.encoder = eval(self.encoder_type)(config, self.relational_memory)
        self.inputdim = 1*2*1*self.enc_lstm_dim
        ...
        #self.w1 = nn.Parameter(torch.FloatTensor(5, 2048, 10240))
        #self.w_a = torch.rand(5)
        #self.w_a = nn.Parameter(self.w_a/self.w_a.sum())

    def forward(self, s1, s2, memory, memory2, memory_, memory2_):
        # s1 : (s1, s1_len)
        u = self.encoder(s1, memory)
        v = self.encoder(s2, memory2)

        x = np.flip(s1[0].transpose(0,1).cpu().numpy(),1).copy()   #Reverse of copy of numpy array of given tensor
        x = torch.from_numpy(x).transpose(0,1).cuda()

        y = np.flip(s2[0].transpose(0,1).cpu().numpy(),1).copy()   #Reverse of copy of numpy array of given tensor
        y = torch.from_numpy(y).transpose(0,1).cuda()

        s1 = (x, s1[1])
        s2 = (y, s2[1])

        u_ = self.encoder(s1, memory_)
        v_ = self.encoder(s2, memory2_)
        ...
        features = features.unsqueeze(1).view(5, bs, 2048)
        features = torch.bmm(features, self.w1)
        features = features.sum(0).view(bs,1,10240)
        outputs = [kappa * features for kappa in self.w_kp]
        outputs = torch.cat(outputs, dim=0).view(5, -1, 2048)
        outputs = self.w_a.view(-1, 1, 1) * outputs
        features = torch.sum(outputs, dim=0).view(bs, 10240)
        '''
        ...
        outputs = torch.cat(outputs, dim=0).view(5, -1, 2 * self.enc_lstm_dim)
        features = torch.sum(outputs, dim=0).view(bs, 2 * self.enc_lstm_dim)
