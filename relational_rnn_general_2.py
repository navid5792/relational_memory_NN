...
class RelationalMemory(nn.Module):
    ...
    def __init__(self, mem_slots, head_size, input_size, num_heads=1, num_blocks=1, forget_bias=1., input_bias=0.,
        ...
        self.mem_size = self.head_size * self.num_heads # 1024
        #self.w_a = torch.rand(mem_slots)
        ...
        self.value_size = self.head_size # 128
        # total size for query-key-value
        self.qkv_size = 2 * self.key_size + self.value_size # 2X128 + 128 = 3X128
        self.total_qkv_size = self.qkv_size * self.num_heads  # denoted as F #3X128X8
        ...
        self.qkv_layernorm = LayerNormalization(self.total_qkv_size)
        ...
        self.attended_memory_layernorm = LayerNormalization(self.mem_size)
        self.attended_memory_layernorm2 = LayerNormalization(self.mem_size)
    ...
    def forward_step(self, inputs, memory, treat_input_as_matrix=True):
        """
        ...
        #print("Next mem: ", next_memory.size())


        # cut out the concatenated input vectors from the original memory slots
        n = inputs_reshape.shape[1]
        next_memory = next_memory[:, :-n, :]
        #print("chopped memory: ", next_memory.size(), "new input: ", inputs_reshape.size(), "new memory: ", memory.size())
        if n >= 2:
            inputs_reshape = inputs_reshape[:, -1, :].view(memory.size())
    ...
    def forward(self, inputs, memory):
        ...
        for idx_step in range(inputs.shape[1]):
            logit, memory = self.forward_step(inputs[:, 0:idx_step+1], memory)

            #outputs = self.w_a.view(-1, 1, 1) * memory.transpose(0,1)

            #outputs = torch.sum(memory, dim=0).view(memory.shape[1], -1, memeory.shape[2])

            logits.append(memory)
        logits = torch.cat(logits, dim = 1 )

        if self.return_all_outputs:
            #return outputs
            return memory.view(memory.shape[0], -1) 
